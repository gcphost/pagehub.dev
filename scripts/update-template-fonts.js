  ...
  if (node.props?.root?.fontFamily) {
    ...
    if (shouldReplace) {
      ...
      const isLikelyHeader =
        ...
        (isTextComponent &&
          node.props?.mobile?.fontSize &&
          ["text-4xl", "text-5xl", "text-6xl", "text-7xl", "text-8xl"].some(
            (size) => node.props.mobile.fontSize.includes(size),
          ));
      ...
      if (isLikelyHeader) {
        ...
        console.log(
          `  ✓ Removed fontFamily from "${displayName || "Text"}" (inherits from body)`,
        );
      }
    }
  }
