...
const updateFlexDirection = (node) => {
  ...
  if (node.props.mobile && node.props.mobile.flexDirection === "flex-row") {
    console.log(
      `📱 Found flex-row in mobile for: ${node.custom?.displayName || node.type?.resolvedName || "Unknown"}`,
    );
...
const processTemplateFile = (filePath) => {
  try {
      ...
      json.templates.forEach((template) => {
        ...
        const processNode = (node) => {
          ...
          if (
            node.props &&
            node.props.mobile &&
            node.props.mobile.flexDirection === "flex-row"
          ) {
            console.log(
              `    📱 Found flex-row in mobile for: ${node.custom?.displayName || node.type?.resolvedName || "Unknown"}`,
            );
            ...
            console.log(
              `    ✅ Updated: mobile → flex-col, desktop → flex-row`,
            );
            updatedCount++;
          }
    ...
    if (updatedCount > 0) {
      ...
      console.log(
        `  ✅ Updated ${updatedCount} flex-direction properties in ${path.basename(filePath)}`,
      );
    } else {
      console.log(
        `  ℹ️  No flex-direction updates needed in ${path.basename(filePath)}`,
      );
    }
  } catch (error) {
